...
{
    [Serializable]
    class Student : Person, IEnumerable, INotifyPropertyChanged
    {
        ...
        public new Student DeepCopy()
        {
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                student = (Student)binaryFormatter.Deserialize(memoryStream);
            }
            return student;
        }
        ...
        public bool Load(string filename)
        {
            ...
            try
            {
                using (FileStream stream = new FileStream(filename, FileMode.Open))
                {
                    ...
                    listExam.Clear();
                    listTest.Clear();
                    foreach (Exam i in student.listExam)
                        this.AddExams((Exam)i.DeepCopy());
                    foreach (Test i in student.listTest)
                        this.AddTest((Test)i.DeepCopy());
                }
                return true;
            }
            catch
        ...
        public static bool Load(string filename, ref Student obj)
        {
            ...
            try
            {
                using (FileStream stream = new FileStream(filename, FileMode.Open))
                {
                    obj = (Student)binaryFormatter.Deserialize(stream);
                }
                return true;
            }
            catch
        ...
        public bool AddFromConsole()
        {
            try
            {
                Console.WriteLine("To add an exam, enter a string in the format: Subject Mark YYYY.MM.DD");
                string str = Console.ReadLine();
                string[] string_exam = str.Split(' ');
                Exam exam = new Exam(string_exam[0], Convert.ToDouble(string_exam[1]), Convert.ToDateTime(string_exam[2]));
                this.AddExams(exam);
                return true;
            }
            catch
