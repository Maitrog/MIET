...
void Teacher::find_by_full_name(Teacher* teacher_base, Name _name)
{
	int count = 0;
...
void sort_by_full_name(Teacher* teacher_base, int count)
{
	...
	for (int step = size / 2; step > 0; step /= 2)
	{
		...
		{
			for (int j = i - step; j >= 0 && ((teacher_base[j].fullName.get_surnamr() + teacher_base[j].fullName.get_name() + teacher_base[j].fullName.get_secondname()) > 
				(teacher_base[j + step].fullName.get_surnamr() + teacher_base[j + step].fullName.get_name() + teacher_base[j + step].fullName.get_secondname())); j -= step)
			{
...
{
	if (fullName == teacher.fullName && department == teacher.department && status == teacher.status)
		return true;
	return false;
}