...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) override {
        ...
        double x2 = inputs[10][0];
        double y2 = inputs[11][0];
        double I2 = inputs[12][0];
        double f2 = inputs[13][0];
        ...
        Field(xb, xe, N, yb, ye, M, x1, y1, I1, f1, x2, y2, I2, f2, x, y, z);
...
private:
    void checkArguments(matlab::mex::ArgumentList& outputs, matlab::mex::ArgumentList& inputs) {
        ...
        if (inputs.size() != 14) {
            matlabPtr->feval(u"error",
                0,
                std::vector<matlab::data::Array>({ factory.createScalar("Fourteen inputs required: xb, xe, N, yb, ye, M, x1, y1, I1, f1, x2, y2, I2, f2.") }));
        }
    ...
    void Field(double xb, double xe, int N, double yb, double ye, int M,
               ...
               double x2, double y2, double I2, double f2,
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                ...
                double r2 = std::sqrt((x[i] - x2) * (x[i] - x2) + (y[j] - y2) * (y[j] - y2));
                double c1, c2;
                ...
                if (r2 < r0) {
                    ...
                    c2 = -I2 / r2 * ((1 / (k * r2)) * std::cos(k * r2 + f2) + 
                                     (1 - 1 / (k * r2) / (k * r2)) * std::sin(k * r2 + f2));
                }

                z[i][j] = c1 + c2; // Суммируем поля от обеих антенн
            }
        }
    }
};
