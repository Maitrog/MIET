    ...
    internal class StrategyFiveTransport : Strategy
    {
        public override Track Execute(int[] path, int volume)
        {
            ...
            foreach (var item in Constants._table)
            {
                ...
                {
                    Plane plane = new Plane(Constants._tableCost[item.Value * 3], Constants._matrixDist[path[1], path[2]]);
                    track.Add(plane);
                }
            }
            ...
            foreach (var item in Constants._table)
            {
                if (item.Key.Contains(Constants._points[path[3]]) && Constants._points[path[3]].Contains("AP"))
                {
                    Plane plane = new Plane(Constants._tableCost[item.Value * 3], Constants._matrixDist[path[3], path[4]]);
                    track.Add(plane);
                }
                if (item.Key.Contains(Constants._points[path[3]]) && Constants._points[path[3]].Contains("TS"))
                {
                    Train train = new Train(Constants._tableCost[item.Value * 3 + 1], Constants._matrixDist[path[3], path[4]]);
                    track.Add(train);
                }
            }
